...
<%-- <div class="sy-info1" style="border-bottom:4px solid #E6E6E6;">
        <ul class="info-img">
        	<c:forEach items="${navigationList }" var="list">
        	 	<li>
	            	<a href="${list.navigation_url}" class="img-1" ><img src="${list.navigation_img}" style="width:42px;height: 42px"> </a>
	                <a href="${list.navigation_url}" class="img-txt">${list.navigation_name}</a> 
            	</li>
        	</c:forEach>
   ...
   <div class="weui-grids" id="category" style="position: static;">
	<c:forEach items="${navigationList}" var="list">
		<a href="${list.navigation_url}" class="weui-grid"><div class="weui-grid__icon"><img src="${list.navigation_img}"></div><p class="weui-grid__label">${list.navigation_name}</p></a>
...
<%--     <c:forEach items="${ctgList}" var="ctgList" varStatus="s">
    ...
    <div class="wy-Module-tit-line"><span>热门推荐</span></div>
    ...
    <ul class="wy-pro-list clear">
       ...
       <li class="div_1">
          <a href="goodsListById.html?goods_id=${goodsList.goods_id}">
            <div class="proimg"><img src="${goodsList.goods_img}"></div>
            <div class="protxt">
              <div class="name">${goodsList.goods_name}</div>
              <div class="wy-pro-pri">¥<span class="span_left">${goodsList.goods_price}</span></div>
            </div>
          </a>
        </li> 
        </c:forEach>
